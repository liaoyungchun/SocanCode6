    ...
    internal class AccessSchema : ISchema
    {
        public Model.Database GetSchema(string connectionString, Model.DatabaseTypes type)
        {
            ...
            foreach (DataRow rt in dtAllTable.Rows)
            {
                ...
                DataTable dtColumns = GetDbSchema(database.ConnectionString, OleDbSchemaGuid.Columns, new object[] { null, null, table.Name });
                foreach (DataRow rc in dtColumns.Rows)
                {
                    Model.Field field = GetField(database.ConnectionString, table.Name, rc);
            ...
            foreach (DataRow rv in dtAllView.Rows)
            {
                ...
                DataTable dtColumns = GetDbSchema(database.ConnectionString, OleDbSchemaGuid.Columns, new object[] { null, null, view.Name });
                foreach (DataRow rc in dtColumns.Rows)
                {
                    Model.Field field = GetField(database.ConnectionString, view.Name, rc);
                    view.AddField(field);
                }
                view.Fields.Sort();
